...
export function TripsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['trips', plate, status, page],
    queryFn: async () => (await api.get('/trips', { params: { plate: plate || undefined, status: status || undefined, page, limit: 20 } })).data,
  });
              ...
              <tbody>
                {rows.map((item: any) => (
                  <tr key={item.id} className="border-t border-slate-100">
                    <td>{item.plate}</td><td>{item.vehicle?.categoryType || '-'}</td><td>{item.startLocal?.name}</td><td>{item.endLocal?.name || '-'}</td><td>{formatDateTime(item.startedAt)}</td><td>{formatDateTime(item.endedAt)}</td><td><Badge>{item.currentStatus}</Badge></td><td>{item.severity}</td>
                  </tr>
                ))}
              </tbody>
            </Table>
